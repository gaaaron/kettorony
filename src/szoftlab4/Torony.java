...
public class Torony implements Aktiv, Mezorevalo {
	...
	// A lövedék tulajdonságai függnek attól, hogy milyen kövekkel van
	// felszerelve a torony, és azok számától
	private Lovedek createLovedek() {
		...
		for (int i = 0; i < kovek.size(); i++) {
			if (kovek.get(i) instanceof Barnavarazsko)
				barna++;
			if (kovek.get(i) instanceof Kekvarazsko)
				kek++;
			if (kovek.get(i) instanceof Narancsvarazsko)
				narancs++;
			if (kovek.get(i) instanceof Pirosvarazsko)
				piros++;
		}
		...
		// TODO: if(index == 1 || specprojectile)
		if (specprojectile)
			lov.hasit = true;
		else
			lov.hasit = false;
	...
	// Amennyiben a tick() meghívja, a torony tüzelni fog. Elõször létrehoz egy
	// lövedéket, majd
	// átadja azt a sebezni kívánt ellenségeknek. (Akik hatótávolságon belül
	// találhatók)
	public void tuzel() {
	...
	// Lekérdezi a közelben(hatótávon belül) található ellenségeket. Ezek után
	// visszadja azt.
	// Hogy meddig lát el, az függ attól, hogy milyen kövekkel van felszerelve,
	// és van-e köd a tornyon
